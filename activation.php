...
//	$sql = "SELECT * FROM users WHERE id='$id' AND username='$u' AND email='$e' AND password='$p' LIMIT 1";
  ...
  $query = mysqli_query($db_conx, $sql);
	...
	if($numrows == 0){
		...
		header("location: message?mes=Your credentials are not matching anything in our system");
	exit();
	}
    ...
    if($numrows == 0){
        ...
        header("location: message?mes=activation_failure");
    	exit();
    } else if($numrows == 1) {
		// Great everything went fine with activation!
        header("location: message?mes=activation_success");
    	exit();
    }
} else {
	// Log this issue of missing initial $_GET variables
	header("location: message?mes=missing_GET_variables");
    exit(); 
}
?>
